...
if ($conn->connect_error) {
  trigger_error('Database connection failed: '  . $conn->connect_error, E_USER_ERROR);
    ...
    $buffer = "id_user".$separator ."nama_user".$separator ."jenis_kelamin".$separator ."tgl_lahir".$separator."username".$separator."password".$separator."status".$separator."email".$separator."keterangan".$separator; 
    $buffer .= $newline; 
    
  $sql="select `id_user`,`nama_user`,`jenis_kelamin`,`tgl_lahir`,`alamat`,`telepon`,`username`,`password`,`status`,`email`,`keterangan` from `$tbuser` order by `id_user` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
